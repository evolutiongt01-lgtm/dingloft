(() => {
  ...
  // Defense in depth: no multitrack can enter the cart until Worker confirms delivery.
  ...
  async function syncCatalog(){
    ...
    try{
      ...
      for(const p of products){
        byKey.set(slugify(p.sku),p); byKey.set(slugify(p.name),p);
      }
      for(const item of tracks){
        const card=document.getElementById(`mt-${item.id}`); if(!card) continue;
        const button=card.querySelector('.mt-cart-action'); if(!button) continue;
        const product=byKey.get(slugify(item.title)) || byKey.get(slugify(item.id));
      ...
      catalogFailed=true; catalogReady=true;
  ...
  // Preserve SKU when the legacy cart handler builds the item object.
  document.addEventListener('click', event=>{
