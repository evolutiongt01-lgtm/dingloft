/* Dingloft Support · Customer realtime chat · v2.7 · Miami sessions + employee + payment request */
...
let user=null,supportData=null,chatState={},chatExists=false,chatUnsub=null,msgUnsub=null,adminPresenceUnsub=null,lastMessages=[],remoteCountdownTimer=null;
...
async function saveRemoteSession(preferredTool,accessCode,showConfirmation){
  try{const d=await api("/support/remote-session",{method:"POST",body:{preferredTool,accessCode}});chatState.remoteSession=d.remoteSession||chatState.remoteSession;renderRemoteSession();if(showConfirmation){const b=document.getElementById("dlRemoteSave");if(b){b.textContent="Guardado";setTimeout(()=>{if(b)b.textContent="Guardar"},1300)}}}catch(e){alert(e.message||"No se pudo guardar el acceso remoto")}
}
