...
if(!DESKTOP)return;
const file=(location.pathname.split('/').filter(Boolean).pop()||'ventas.html').toLowerCase();
if(file==='desktop-shell.html')return;
const src=`${location.pathname.split('/').filter(Boolean).pop()||'ventas.html'}${location.search}${location.hash}`;
