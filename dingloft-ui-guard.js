(() => {
  ...
  function setupDirectMobileProduct(){
    ...
    style.textContent=`
      ...
      #dlDirectDock{position:fixed;z-index:2147481001;left:max(10px,env(safe-area-inset-left));right:max(10px,env(safe-area-inset-right));bottom:calc(5px + env(safe-area-inset-bottom));height:66px;padding:6px 7px;display:grid;grid-template-columns:1fr 1fr 76px 1fr 1fr;align-items:stretch;border:1px solid rgba(255,255,255,.12);border-radius:22px;background:linear-gradient(145deg,rgba(18,24,31,.92),rgba(6,9,13,.965));box-shadow:0 20px 58px rgba(0,0,0,.62),inset 0 1px 0 rgba(255,255,255,.075);backdrop-filter:blur(28px) saturate(165%);-webkit-backdrop-filter:blur(28px) saturate(165%)}
      #dlDirectDock .dd-item{height:52px;border:0;background:transparent;color:#6f7c8d;text-decoration:none;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;border-radius:17px;font:720 .57rem/1 -apple-system,BlinkMacSystemFont,'SF Pro Display',sans-serif;-webkit-tap-highlight-color:transparent}#dlDirectDock .dd-item svg{width:20px;height:20px;stroke:currentColor;fill:none;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}#dlDirectDock .dd-item.catalog{color:#edf8ff;background:rgba(255,255,255,.04)}#dlDirectDock .dd-item.catalog svg{color:#72dcff;filter:drop-shadow(0 0 8px rgba(114,220,255,.25))}#dlDirectDock .dd-cart-slot{position:relative;display:flex;justify-content:center}#dlDirectDock .dd-cart{position:absolute;top:-9px;width:60px;height:60px;border-radius:20px;border:6px solid #05080c;background:#f4f7f9;color:#05080c;display:grid;place-items:center;box-shadow:0 14px 32px rgba(0,0,0,.52);-webkit-tap-highlight-color:transparent}#dlDirectDock .dd-cart svg{width:27px;height:27px;stroke:currentColor;fill:none;stroke-width:1.8}#dlDirectDock .dd-cart-label{position:absolute;top:50px;color:#6f7c8d;font:720 .57rem/1 -apple-system,BlinkMacSystemFont,'SF Pro Display',sans-serif}
      ...
      @media(max-width:390px){#dlDirectDock .dd-item{font-size:.53rem}}
    `;
    ...
    const dock=document.createElement('nav');dock.id='dlDirectDock';dock.setAttribute('aria-label','Navegación Dingloft');dock.innerHTML=`
      ...
      <a class="dd-item catalog" href="/app.html?route=catalog" aria-label="Catálogo"><svg viewBox="0 0 24 24"><path d="M4 5h16v14H4z"></path><path d="M8 9h8M8 13h8M8 17h5"></path></svg><span>Catálogo</span></a>
      <span class="dd-cart-slot"><button class="dd-cart" id="dlDirectCart" type="button" aria-label="Abrir carrito"><svg viewBox="0 0 24 24"><path d="M6.5 8.5h11l-1 11h-9z"></path><path d="M9 8.5V7a3 3 0 0 1 6 0v1.5"></path></svg></button><span class="dd-cart-label">Carrito</span></span>
