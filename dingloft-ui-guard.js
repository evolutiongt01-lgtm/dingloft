(() => {
  ...
  function setupDirectMobileProduct(){
    ...
    if(!document.getElementById('dlDockMasterCss')){const l=document.createElement('link');l.id='dlDockMasterCss';l.rel='stylesheet';l.href='/dingloft-mobile-dock.css?v=29';document.head.appendChild(l);}
    ...
    style.textContent=`
      ...
      #dlDirectTop{position:fixed;z-index:2147481000;top:0;left:0;right:0;height:calc(66px + env(safe-area-inset-top));padding:env(safe-area-inset-top) 16px 0;display:flex;align-items:center;justify-content:center;gap:11px;border-bottom:1px solid rgba(255,255,255,.075);background:linear-gradient(180deg,rgba(4,6,9,1),rgba(5,7,10,.97));backdrop-filter:blur(26px) saturate(160%);-webkit-backdrop-filter:blur(26px) saturate(160%);box-shadow:0 10px 34px rgba(0,0,0,.26)}
    ...
    const top=document.createElement('div');top.id='dlDirectTop';top.innerHTML=`<img src="/img/pwa-liquid-rounded-192-v17.png?v=29" alt="Dingloft"><div class="dt-copy"><strong>DINGLOFT</strong><small>Evolution Group</small></div>`;document.body.appendChild(top);
