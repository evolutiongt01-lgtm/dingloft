...
function classify(src){try{const u=new URL(src,location.origin);const file=(u.pathname.split('/').pop()||'index.html').toLowerCase();const trueHome=file==='ventas.html'||file==='index.html'||file==='tienda.html';const productPage=homeFiles.has(file)&&!trueHome;const software=productPage||(trueHome&&u.hash==='#catalogo');return{home:trueHome&&!software,multitracks:file==='multitrack.html',account:file==='account.html',software};}catch(_){return{}}}
const navActions=document.getElementById('desktopNavActions'),navGlider=document.getElementById('navGlider');
const navAdmin=document.getElementById('navAdmin'),navSearch=document.getElementById('navSearch'),navCart=document.getElementById('navCart'),navCartCount=document.getElementById('navCartCount'),navSoftware=document.getElementById('navSoftware');let adminSession=false,currentSrc='';const navButtons=[document.getElementById('navHome'),navSoftware,document.getElementById('navMultitracks'),document.getElementById('navAccount')].filter(Boolean);
...
async function openDesktopSearch(){
  if(!searchOverlay)return;searchOverlay.classList.add('show');searchOverlay.setAttribute('aria-hidden','false');if(searchResults)searchResults.innerHTML='<div class="dl-search-empty"><b>Actualizando catálogo…</b>Buscando productos disponibles.</div>';setTimeout(()=>searchInput?.focus(),70);
...
function closeDesktopSearch(){if(!searchOverlay)return;searchOverlay.classList.remove('show');searchOverlay.setAttribute('aria-hidden','true')}
