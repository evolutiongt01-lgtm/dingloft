...
const navAdmin=document.getElementById('navAdmin'),navSearch=document.getElementById('navSearch'),navSoftware=document.getElementById('navSoftware');let adminSession=false,currentSrc='';const navButtons=[document.getElementById('navHome'),navSoftware,document.getElementById('navMultitracks'),document.getElementById('navAccount')].filter(Boolean);
...
function isTrueHome(src){try{const u=new URL(src,location.origin),file=(u.pathname.split('/').pop()||'index.html').toLowerCase();return file==='ventas.html'||file==='index.html'||file==='tienda.html'}catch(_){return false}}function syncAdminNav(){if(!navAdmin)return;navAdmin.hidden=!adminSession}function paint(src){currentSrc=src;const c=classify(src);document.getElementById('navHome')?.classList.toggle('active',!!c.home);navSoftware?.classList.toggle('active',!!c.software);document.getElementById('navMultitracks')?.classList.toggle('active',!!c.multitracks);document.getElementById('navAccount')?.classList.toggle('active',!!c.account);syncAdminNav()}
